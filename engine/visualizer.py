...
def draw_network(net: Network, show_weights: bool = True,
                 node_size: int = 800, figsize=(12, 8)):
    """
    绘制网络图，节点颜色表示激活值（红高蓝低），边宽表示权重大小。
    """
    ...
    for edge in net._all_edges:
        ...
        weight = edge.weight
        linewidth = 1 + 3 * abs(weight) / max(1, max(e.weight for e in net._all_edges))
        alpha = min(0.8, 0.3 + abs(weight))
        ...
        if show_weights:
            ...
            plt.text(mid_x, mid_y, f"{weight:.2f}", fontsize=8, ha='center', va='center',
    ...
    for node in net._all_nodes:
        ...
        act = node.activation
        # 颜色映射：激活值大 -> 红色，小 -> 蓝色
        color = plt.cm.RdBu(act if act <= 1 else 1.0)
    ...
    plt.title("Neural Network Structure (Color = Activation, Width = Weight)")
    plt.axis('off')
    plt.tight_layout()
    plt.show()